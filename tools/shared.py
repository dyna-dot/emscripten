...
class WindowsPopen(object):
    def __init__(self, args, bufsize=0, executable=None, stdin=None, stdout=None, stderr=None, preexec_fn=None, close_fds=False,
        ...
        if len(args) >= 2 and args[1].endswith("emscripten.py"):
            response_filename = response_file.create_response_file(
                args[2:], TEMP_DIR)
            args = args[0:2] + ['@' + response_filename]

        try:
            # Call the process with fixed streams.
            self.process = subprocess.Popen(args, bufsize, executable, self.stdin_, self.stdout_, self.stderr_,
    ...
    def check_returncode(self):
        if self.returncode != 0:
            raise Py2CalledProcessError(
                returncode=self.returncode, cmd=self.args, output=self.stdout, stderr=self.stderr)


def run_process(cmd, check=True, input=None, universal_newlines=True, *args, **kw):
    kw.setdefault('universal_newlines', True)

    debug_text = '%sexecuted %s' % (
        'successfully ' if check else '', ' '.join(cmd))
...
def generate_config(path, first_time=False):
    ...
    config_file = open(path_from_root(
        'tools', 'settings_template_readonly.py')).read().splitlines()
    config_file = config_file[3:]  # remove the initial comment
    config_file = '\n'.join(config_file)
    # autodetect some default paths
    config_file = config_file.replace(
        '\'{{{ EMSCRIPTEN_ROOT }}}\'', repr(__rootpath__))
    llvm_root = os.path.dirname(find_executable(
        'llvm-dis') or '/usr/bin/llvm-dis')
...
if EM_CONFIG and not os.path.isfile(EM_CONFIG):
    if EM_CONFIG.startswith('-'):
        exit_with_error(
            'Passed --em-config without an argument. Usage: --em-config /path/to/.emscripten or --em-config LLVM_ROOT=/path;...')
    if '=' not in EM_CONFIG:
        exit_with_error('File ' + EM_CONFIG +
                        ' passed to --em-config does not exist!')
    else:
...
def parse_config_file():
    ...
    try:
        ...
        exit_with_error('Error in evaluating %s (at %s): %s, text: %s',
                        EM_CONFIG, CONFIG_FILE, str(e), config_text)
...
parse_config_file()
SPIDERMONKEY_ENGINE = fix_js_engine(
...
if EM_POPEN_WORKAROUND and os.name == 'nt':
    logger.debug(
        'Installing Popen workaround handler to avoid bug http://bugs.python.org/issue3905')
    Popen = WindowsPopen
else:
...
def check_llvm_version():
    ...
    if expected in actual:
        return True
    logger.warning('LLVM version appears incorrect (seeing "%s", expected "%s")' % (
        actual, expected))
    return False
...
def get_llc_targets():
    try:
        llc_version_info = run_process(
            [LLVM_COMPILER, '--version'], stdout=PIPE).stdout
    except Exception as e:
...
def check_llvm():
    ...
    if not Settings.WASM_BACKEND:
        if not has_asm_js_target(targets):
            logger.critical(
                'fastcomp in use, but LLVM has not been built with the JavaScript backend as a target, llc reports:')
            print(
                '===========================================================================', file=sys.stderr)
        ...
        if not has_wasm_target(targets):
            logger.critical(
                'WebAssembly set as target, but LLVM has not been built with the WebAssembly backend, llc reports:')
            print(
                '===========================================================================', file=sys.stderr)
            print(targets, file=sys.stderr)
            print(
    ...
    if not Settings.WASM_BACKEND:
        ...
        if '(emscripten ' not in clang_v:
            logger.error(
                'clang version does not appear to include fastcomp (%s)', str(clang_v))
            return False
        llvm_build_version, clang_build_version = clang_v.split(
            '(emscripten ')[1].split(')')[0].split(' : ')
        if EMSCRIPTEN_VERSION != llvm_build_version or EMSCRIPTEN_VERSION != clang_build_version:
            logger.error('Emscripten, llvm and clang build versions do not match, this is dangerous (%s, %s, %s)',
                         EMSCRIPTEN_VERSION, llvm_build_version, clang_build_version)
            logger.error(
                'Make sure to rebuild llvm and clang after updating repos')
...
def check_node_version():
    ...
    try:
        actual = run_process(
            NODE_JS + ['--version'], stdout=PIPE).stdout.strip()
        version = tuple(map(int, actual.replace(
            'v', '').replace('-pre', '').split('.')))
        if version >= EXPECTED_NODE_VERSION:
            return True
        logger.warning('node version appears too old (seeing "%s", expected "%s")' % (
            actual, 'v' + ('.'.join(map(str, EXPECTED_NODE_VERSION)))))
        return False
    except Exception as e:
...
def check_closure_compiler():
    try:
        ...
        logger.warning('java does not seem to exist, required for closure compiler, which is optional (define JAVA in ' +
                       hint_config_file_location() + ' if you want it)')
        return False
    if not os.path.exists(CLOSURE_COMPILER):
        logger.warning('Closure compiler (%s) does not exist, check the paths in %s' % (
            CLOSURE_COMPILER, EM_CONFIG))
        return False
    return True
...
def perform_sanify_checks():
    ...
    with ToolchainProfiler.profile_block('sanity compiler_engine'):
        if not jsrun.check_engine(COMPILER_ENGINE):
            exit_with_error(
                'The JavaScript shell used for compiling (%s) does not seem to work, check the paths in %s', COMPILER_ENGINE, EM_CONFIG)

    with ToolchainProfiler.profile_block('sanity LLVM'):
        for cmd in [CLANG, LLVM_LINK, LLVM_AR, LLVM_OPT, LLVM_AS, LLVM_DIS, LLVM_NM, LLVM_INTERPRETER]:
            # .exe extension required for Windows
            if not os.path.exists(cmd) and not os.path.exists(cmd + '.exe'):
                exit_with_error(
                    'Cannot find %s, check the paths in %s', cmd, EM_CONFIG)
    ...
    if not os.path.exists(PYTHON) and not os.path.exists(cmd + '.exe'):
        try:
            ...
            exit_with_error(
                'Cannot find %s, check the paths in config file (%s)', PYTHON, CONFIG_FILE)
...
def check_sanity(force=False):
    ...
    with ToolchainProfiler.profile_block('sanity'):
        ...
        if os.path.exists(sanity_file):
            ...
            if sanity_mtime <= settings_mtime:
                ...
                if sanity_data != expected:
                    reason = 'system change: %s vs %s' % (
                        expected, sanity_data)
                elif not force:
                    return  # all is well
        ...
        if reason:
            if FROZEN_CACHE:
                logger.warning(
                    '(Emscripten: %s, cache may need to be cleared, but FROZEN_CACHE is set)' % reason)
            else:
        ...
        if not llvm_ok:
            exit_with_error(
                'failing sanity checks due to previous llvm failure')
...
def macos_find_native_sdk_path():
    try:
        ...
        # Just pick first one found, we don't care which one we found.
        sdk_path = os.path.join(sdk_root, sdks[0])
...
def get_clang_native_args():
    ...
    if MACOS:
        ...
        if sdk_path:
            CACHED_CLANG_NATIVE_ARGS = [
                '-isysroot', macos_find_native_sdk_path()]
    elif os.name == 'nt':
...
def get_clang_native_env():
    ...
    if WINDOWS:
        ...
        if 'VSINSTALLDIR' in env:
            ...
            visual_studio_path = os.path.normpath(
                os.path.join(env['VS140COMNTOOLS'], '../..'))
        elif 'ProgramFiles(x86)' in env:
            visual_studio_path = os.path.normpath(os.path.join(
        ...
        if not os.path.isdir(visual_studio_path):
            raise Exception('Visual Studio 2015 was not found in "' + visual_studio_path +
        ...
        if 'ProgramFiles(x86)' in env:
            ...
            raise Exception(
                'Unable to detect Program files directory for native Visual Studio build!')
        ...
        if 'WindowsSdkDir' in env:
            ...
            windows8_sdk_dir = os.path.join(
                prog_files_x86, 'Windows Kits', '8.1')
        if not os.path.isdir(windows8_sdk_dir):
            raise Exception('Windows 8.1 SDK was not found in "' + windows8_sdk_dir +
        ...
        if os.path.isdir(os.path.join(prog_files_x86, 'Windows Kits', '10')):
            windows10_sdk_dir = os.path.join(
                prog_files_x86, 'Windows Kits', '10')
        if not os.path.isdir(windows10_sdk_dir):
            raise Exception('Windows 10 SDK was not found in "' + windows10_sdk_dir +
        ...
        windows10sdk_kits_include_dir = os.path.join(
            windows10_sdk_dir, 'Include')
        windows10sdk_kit_version_name = [x for x in os.listdir(windows10sdk_kits_include_dir) if os.path.isdir(
            os.path.join(windows10sdk_kits_include_dir, x))][0]  # e.g. "10.0.10150.0" or "10.0.10240.0"
        ...
        append_item('INCLUDE', os.path.join(
            env['VCINSTALLDIR'], 'ATLMFC', 'INCLUDE'))
        append_item('INCLUDE', os.path.join(windows10_sdk_dir,
                                            'include', windows10sdk_kit_version_name, 'ucrt'))
        ...
        append_item('INCLUDE', os.path.join(
            env['VCINSTALLDIR'], 'ATLMFC', 'INCLUDE'))
        append_item('INCLUDE', os.path.join(
            windows8_sdk_dir, 'include', 'shared'))
        append_item('INCLUDE', os.path.join(windows8_sdk_dir, 'include', 'um'))
        append_item('INCLUDE', os.path.join(
            windows8_sdk_dir, 'include', 'winrt'))
        logger.debug('VS2015 native build INCLUDE: ' + env['INCLUDE'])

        append_item('LIB', os.path.join(env['VCINSTALLDIR'], 'LIB', 'amd64'))
        append_item('LIB', os.path.join(
            env['VCINSTALLDIR'], 'ATLMFC', 'LIB', 'amd64'))
        append_item('LIB', os.path.join(windows10_sdk_dir, 'lib',
                                        windows10sdk_kit_version_name, 'ucrt', 'x64'))
        ...
        append_item('LIB', os.path.join(
            windows8_sdk_dir, 'lib', 'winv6.3', 'um', 'x64'))
        logger.debug('VS2015 native build LIB: ' + env['LIB'])

        env['PATH'] = env['PATH'] + ';' + \
            os.path.join(env['VCINSTALLDIR'], 'BIN')
...
LLVM_DWARFDUMP = os.path.expanduser(
...
def get_emscripten_temp_dir():
    ...
    if not EMSCRIPTEN_TEMP_DIR:
        EMSCRIPTEN_TEMP_DIR = tempfile.mkdtemp(
            prefix='emscripten_temp_', dir=configuration.TEMP_DIR)
...
class WarningManager(object):
    ...
    def capture_warnings(cmd_args):
        for i in range(len(cmd_args)):
            ...
            if cmd_args[i] == '-Wwarn-absolute-paths':
                ...
                warning_enum = cmd_args[i].replace(
                    '-Wno-', '').replace('-W', '')
                warning_enum = warning_enum.upper().replace('-', '_')

                if warning_enum in WarningManager.warnings:
                    WarningManager.warnings[warning_enum]['enabled'] = not cmd_args[i].startswith(
                        '-Wno-')
                    cmd_args[i] = ''
    ...
    def warn(warning_type, message=None):
        ...
        if warning['enabled'] and not warning['printed']:
            ...
            logger.warning(
                (message or warning['message']) + ' [-W' + warning_type.lower().replace('_', '-') + ']')
    ...
    def __init__(self, environ=os.environ):
        ...
        try:
            ...
            if self.TEMP_DIR is None:
                logger.critical('TEMP_DIR not defined in ' + hint_config_file_location(
                ) + ", and could not detect a suitable directory! Please configure .emscripten to contain a variable TEMP_DIR='/path/to/temp/dir'.")
            logger.debug('TEMP_DIR not defined in ' +
                         hint_config_file_location() + ', using ' + self.TEMP_DIR)

        if not os.path.isdir(self.TEMP_DIR):
            logger.critical("The temp directory TEMP_DIR='" + self.TEMP_DIR +
                            "' doesn't seem to exist! Please make sure that the path is correct.")
        ...
        if DEBUG:
            try:
                ...
                logger.error(str(
                    e) + 'Could not create canonical temp dir. Check definition of TEMP_DIR in ' + hint_config_file_location())
...
if CLOSURE_COMPILER is None:
    CLOSURE_COMPILER = path_from_root(
        'third_party', 'closure-compiler', 'compiler.jar')

if PYTHON is None:
    logger.debug('PYTHON not defined in ' +
                 hint_config_file_location() + ', using "%s"' % (sys.executable,))
    PYTHON = sys.executable

if JAVA is None:
    logger.debug('JAVA not defined in ' +
                 hint_config_file_location() + ', using "java"')
    JAVA = 'java'
...
def check_vanilla():
    ...
    if 'EMCC_WASM_BACKEND' in os.environ:
        ...
        def get_vanilla_file():
            ...
            open(saved_file, 'w').write(
                ('1' if check_vanilla() else '0') + ':' + LLVM_ROOT)
            return saved_file
        ...
        if CONFIG_FILE and os.path.getmtime(CONFIG_FILE) > os.path.getmtime(is_vanilla_file):
            logger.debug(
                'config file changed since we checked vanilla; re-checking')
            is_vanilla_file = temp_cache.get(
                'is_vanilla.txt', get_vanilla_file, force=True)
        try:
            ...
            logger.debug(
                'failed to use vanilla file, will re-check: ' + str(e))
            is_vanilla = check_vanilla()
        temp_cache = None
        if is_vanilla:
...
def emsdk_opts():
    ...
    c_opts = ['-nostdinc', '-Xclang',
              '-nobuiltininc', '-Xclang', '-nostdsysteminc']
...
def to_cc(cxx):
    ...
    basename = basename.replace(
        'clang++', 'clang').replace('g++', 'gcc').replace('em++', 'emcc')
...
def expand_byte_size_suffixes(value):
    ...
    integer.
    """
    value = value.lower().replace('tb', '*1024*1024*1024*1024').replace('gb',
                                                                        '*1024*1024*1024').replace('mb', '*1024*1024').replace('kb', '*1024').replace('b', '')
    try:
        return eval(value)
    except:
...
class SettingsManager(object):
    ...
    class __impl(object):
        ...
        def reset(self):
            ...
            settings = open(path_from_root('src', 'settings.js')
                            ).read().replace('//', '#')
            ...
            for legacy in self.attrs['LEGACY_SETTINGS']:
                if len(legacy) == 2:
                    ...
                    self.legacy_settings[name] = (
                        None, 'setting renamed to ' + new_name)
        ...
        def __getattr__(self, attr):
            if attr in self.attrs:
                ...
                raise AttributeError(
                    "Settings object has no attribute '%s'" % attr)
        ...
        def __setattr__(self, attr, value):
            ...
            if attr in self.legacy_settings:
                if self.attrs['STRICT']:
                    exit_with_error(
                        'legacy setting used in strict mode: %s', attr)
                fixed_values, error_message = self.legacy_settings[attr]
                if fixed_values and value not in fixed_values:
                    exit_with_error('Invalid command line option -s ' +
                                    attr + '=' + str(value) + ': ' + error_message)
                else:
                    logger.debug('Option -s ' + attr + '=' + str(value) +
                                 ' has been removed from the codebase. (' + error_message + ')')
            ...
            if attr not in self.attrs:
                logger.error(
                    'Assigning a non-existent settings attribute "%s"' % attr)
                suggestions = ', '.join(difflib.get_close_matches(
                    attr, list(self.attrs.keys())))
                if suggestions:
...
def verify_settings():
    ...
    if Settings.WASM_BACKEND:
        ...
        if Settings.CYBERDWARF:
            exit_with_error(
                'emcc: CYBERDWARF is not supported by the LLVM wasm backend')

        if Settings.EMTERPRETIFY:
            exit_with_error(
                'emcc: EMTERPRETIFY is not supported by the LLVM wasm backend')

        if not os.path.exists(WASM_LD) or run_process([WASM_LD, '--version'], stdout=PIPE, stderr=PIPE, check=False).returncode != 0:
            exit_with_error(
                'emcc: WASM_BACKEND selected but could not find lld (wasm-ld): %s', WASM_LD)

        if Settings.EMULATED_FUNCTION_POINTERS:
            exit_with_error(
                'emcc: EMULATED_FUNCTION_POINTERS is not meaningful with the wasm backend.')
...
def extract_archive_contents(archive_file):
    lines = run_process([LLVM_AR, 't', archive_file],
                        stdout=PIPE).stdout.splitlines()
    # ignore empty lines
    contents = [l for l in lines if len(l)]
    if len(contents) == 0:
        logger.debug(
            'Archive %s appears to be empty (recommendation: link an .so instead of .a)' % archive_file)
        return {
            'returncode': 0,
            'dir': None,
            'files': []
    ...
    proc = run_process([LLVM_AR, 'xo', archive_file],
                       stdout=PIPE, stderr=STDOUT, cwd=temp_dir)
    ...
    if missing_contents:
        exit_with_error('llvm-ar failed to extract file(s) ' + str(missing_contents) +
                        ' from archive file ' + f + '! Error:' + str(proc.stdout))
...
class Building(object):
    COMPILER = CLANG
    # Used to pass the JS engine override from runner.py -> test_benchmark.py
    ...
    def get_multiprocessing_pool():
        if not Building.multiprocessing_pool:
            ...
            if cores == 1:
                ...
                Building.multiprocessing_pool = multiprocessing.Pool(
                    processes=cores, initializer=g_multiprocessing_initializer, initargs=child_env)
    ...
    def get_building_env(native=False, doublequote_commands=False, cflags=[]):
        ...
        if native:
            ...
            EMSCRIPTEN_MODIFIES = ['LDSHARED', 'AR',
                                   'CROSS_COMPILE', 'NM', 'RANLIB']
            for dangerous in EMSCRIPTEN_MODIFIES:
                if env.get(dangerous) and env.get(dangerous) == non_native.get(dangerous):
                    # better to delete it than leave it, as the non-native one is definitely wrong
                    del env[dangerous]
            return env
        ...
        env['CC'] = quote(unsuffixed(
            EMCC)) if not WINDOWS else 'python %s' % quote(EMCC)
        env['CXX'] = quote(unsuffixed(
            EMXX)) if not WINDOWS else 'python %s' % quote(EMXX)
        env['AR'] = quote(unsuffixed(
            EMAR)) if not WINDOWS else 'python %s' % quote(EMAR)
        env['LD'] = quote(unsuffixed(
            EMCC)) if not WINDOWS else 'python %s' % quote(EMCC)
        env['NM'] = quote(LLVM_NM)
        env['LDSHARED'] = quote(unsuffixed(
            EMCC)) if not WINDOWS else 'python %s' % quote(EMCC)
        env['RANLIB'] = quote(unsuffixed(
            EMRANLIB)) if not WINDOWS else 'python %s' % quote(EMRANLIB)
        ...
        # if set to nothing, CFLAGS is used, which we don't want
        env['HOST_CFLAGS'] = "-W"
        # if set to nothing, CXXFLAGS is used, which we don't want
        env['HOST_CXXFLAGS'] = "-W"
        env['PKG_CONFIG_LIBDIR'] = path_from_root(
            'system', 'local', 'lib', 'pkgconfig') + os.path.pathsep + path_from_root('system', 'lib', 'pkgconfig')
        env['PKG_CONFIG_PATH'] = os.environ.get('EM_PKG_CONFIG_PATH', '')
        env['EMSCRIPTEN'] = path_from_root()
        env['PATH'] = path_from_root(
            'system', 'bin') + os.pathsep + env['PATH']
    ...
    def remove_sh_exe_from_path(env):
        ...
        if not WINDOWS:
            return env
        path = env['PATH'].split(';')
        path = [p for p in path if not os.path.exists(
            os.path.join(p, 'sh.exe'))]
        env['PATH'] = ';'.join(path)
        return env
    ...
    def handle_CMake_toolchain(args, env):
        ...
        if not has_substr(args, '-DCMAKE_TOOLCHAIN_FILE'):
            args.append('-DCMAKE_TOOLCHAIN_FILE=' + path_from_root('cmake',
    ...
    def configure(args, stdout=None, stderr=None, env=None, cflags=[]):
        ...
        try:
            ...
            if EM_BUILD_VERBOSE >= 1:
                stderr = None
            res = run_process(args, check=False, stdout=stdout,
        ...
        if res.returncode is not 0:
            logger.error('Configure step failed with non-zero return code: %s.  Command line: %s at %s' %
                         (res.returncode, ' '.join(args), os.getcwd()))
            raise subprocess.CalledProcessError(
                cmd=args, returncode=res.returncode)
    ...
    def make(args, stdout=None, stderr=None, env=None, cflags=[]):
        ...
        try:
            ...
            if EM_BUILD_VERBOSE >= 1:
                stderr = None
            res = run_process(args, stdout=stdout, stderr=stderr,
                              env=env, shell=WINDOWS, check=False)
        except Exception:
        ...
        with ToolchainProfiler.profile_block('parallel_llvm_nm'):
            ...
            for i, file in enumerate(files):
                if object_contents[i].returncode != 0:
                    logger.debug('llvm-nm failed on file ' + file + ': return code ' + str(
                        object_contents[i].returncode) + ', error: ' + object_contents[i].output)
    ...
    def read_link_inputs(files):
        with ToolchainProfiler.profile_block('read_link_inputs'):
            ...
            pool = Building.get_multiprocessing_pool()
            object_names_in_archives = pool.map(
                extract_archive_contents, archive_names)
            ...
            for n in range(len(archive_names)):
                if object_names_in_archives[n]['returncode'] != 0:
                    raise Exception(
                        'llvm-ar failed on archive ' + archive_names[n] + '!')
                Building.ar_contents[archive_names[n]
                                     ] = object_names_in_archives[n]['files']
                clean_temporary_archive_contents_directory(
                    object_names_in_archives[n]['dir'])
    ...
    def llvm_backend_args():
        ...
        if Settings.DISABLE_EXCEPTION_CATCHING == 2:
            whitelist = ','.join(
                Settings.EXCEPTION_CATCHING_WHITELIST or ['__fake'])
    ...
    def link_llvm(linker_inputs, target):
        ...
        assert os.path.exists(target) and (
            output is None or 'Could not open input file' not in output), 'Linking error: ' + output
        return target
    ...
    def link(files, target, force_archive_contents=False, temp_files=None, just_calculate=False):
        ...
        unresolved_symbols = set([func[1:]
                                  for func in Settings.EXPORTED_FUNCTIONS])
        ...
        for f in files:
            if not f.startswith('-'):
                has_ar = has_ar or Building.is_ar(
                    Building.make_paths_absolute(f))
        ...
        def consider_object(f, force_add=False):
            ...
            if not new_symbols.is_valid_for_nm():
                logger.warning(
                    'object %s is not valid according to llvm-nm, cannot link' % (f))
                return False
            # Check the object is valid for us, and not a native object file.
            if not Building.is_bitcode(f):
                logger.warning(
                    'object %s is not a valid object file for emscripten, cannot link' % (f))
                return False
            ...
            if do_add:
                ...
                unresolved_symbols.update(
                    new_symbols.undefs.difference(resolved_symbols))
        ...
        for f in files:
            ...
            if f.startswith('-'):
                ...
                if Building.is_bitcode(absolute_path_f):
                    ...
                    logging.debug(
                        'ignoring non-bitcode file for link: %s' % absolute_path_f)
            else:
                # Extract object files from ar archives, and link according to gnu ld semantics
                # (link in an entire .o from the archive if it supplies symbols still unresolved)
                consider_archive(
                    absolute_path_f, in_whole_archive or force_add_all)
        ...
        if current_archive_group:
            logger.debug(
                '--start-group without matching --end-group, rescanning')
            scan_archive_group(current_archive_group)
            current_archive_group = None
        ...
        if not just_calculate:
            logger.debug('emcc: Building.linking: %s to %s',
                         actual_files, target)
        ...
        try:
            ...
            for i in inputs:
                if not os.path.exists(i):
                    logger.warning('Note: Input file "' +
                                   i + '" did not exist.')
                elif not Building.is_bitcode(i):
                    logger.warning(
                        'Note: Input file "' + i + '" exists but was not an LLVM bitcode file suitable for Emscripten. Perhaps accidentally mixing native built object files with Emscripten?')
    ...
    def llvm_dis(input_filename, output_filename):
        ...
        output = run_process(
            [LLVM_DIS, input_filename, '-o', output_filename], stdout=PIPE).stdout
        assert os.path.exists(
            output_filename), 'Could not create .ll file: ' + output
    ...
    def llvm_as(input_filename, output_filename):
        ...
        try_delete(output_filename)
        output = run_process(
            [LLVM_AS, input_filename, '-o', output_filename], stdout=PIPE).stdout
        assert os.path.exists(
            output_filename), 'Could not create bc file: ' + output
    ...
    def llvm_nm_uncached(filename, stdout=PIPE, stderr=PIPE, include_internal=False):
        ...
        proc = run_process([LLVM_NM, filename], stdout=stdout,
    ...
    def llvm_nm(filename, stdout=PIPE, stderr=PIPE, include_internal=False):
        ...
        ret = Building.llvm_nm_uncached(
            filename, stdout, stderr, include_internal)

        if ret.returncode != 0:
            logger.debug('llvm-nm failed on file ' + filename +
                         ': return code ' + str(ret.returncode) + ', error: ' + ret.output)
    ...
    def emcc(filename, args=[], output_filename=None, stdout=None, stderr=None, env=None):
        ...
        try_delete(output_filename)
        run_process([PYTHON, EMCC, filename] + args + ['-o',
                                                       output_filename], stdout=stdout, stderr=stderr, env=env)
    ...
    def emar(action, output_filename, filenames, stdout=None, stderr=None, env=None):
        ...
        response_filename = response_file.create_response_file(
            filenames, TEMP_DIR)
        cmd = [PYTHON, EMAR, action, output_filename] + \
            ['@' + response_filename]
        try:
        ...
        if 'c' in action:
            assert os.path.exists(
                output_filename), 'emar could not create output file: ' + output_filename
    ...
    def get_safe_internalize():
        ...
        if len(internalize_list) > 8192:
            logger.debug(
                'using response file for EXPORTED_FUNCTIONS in internalize')
    ...
    def js_optimizer(filename, passes, debug=False, extra_info=None, output_filename=None, just_split=False, just_concat=False):
        ...
        ret = js_optimizer.run(filename, passes, NODE_JS,
                               debug, extra_info, just_split, just_concat)
        if output_filename:
    ...
    def eval_ctors(js_file, binary_file, binaryen_bin='', debug_info=False):
        cmd = [PYTHON, path_from_root('tools', 'ctor_evaller.py'), js_file, binary_file, str(Settings.TOTAL_MEMORY), str(
            Settings.TOTAL_STACK), str(Settings.GLOBAL_BASE), binaryen_bin, str(int(debug_info))]
        if binaryen_bin:
    ...
    def calculate_reachable_functions(infile, initial_list, can_reach=True):
        with ToolchainProfiler.profile_block('calculate_reachable_functions'):
            ...
            Building.js_optimizer(
                infile, ['dumpCallGraph'], output_filename=temp, just_concat=True)
            ...
            for name, funcs in asm.tables.items():
                can_call[name] = set([x.strip()
                                      for x in funcs[1:-1].split(',')])
            # print can_call
    ...
    def closure_compiler(filename, pretty=True, advanced=True):
        with ToolchainProfiler.profile_block('closure_compiler'):
            ...
            CLOSURE_ANNOTATIONS = [
                '--js', path_from_root('src', 'closure-annotations.js')]
            ...
            if not Settings.ASMFS:
                ...
                if Settings.FILESYSTEM:
                    CLOSURE_ANNOTATIONS += ['--js', path_from_root(
            ...
            NODE_EXTERNS_BASE = path_from_root(
                'third_party', 'closure-compiler', 'node-externs')
            NODE_EXTERNS = os.listdir(NODE_EXTERNS_BASE)
            NODE_EXTERNS = [os.path.join(NODE_EXTERNS_BASE, name) for name in NODE_EXTERNS
                            if name.endswith('.js')]
            NODE_EXTERNS = [path_from_root(
                'src', 'node-externs.js')] + NODE_EXTERNS
            V8_EXTERNS = [path_from_root('src', 'v8-externs.js')]
            SPIDERMONKEY_EXTERNS = [path_from_root(
                'src', 'spidermonkey-externs.js')]
            BROWSER_EXTERNS_BASE = path_from_root(
                'third_party', 'closure-compiler', 'browser-externs')
            ...
            args = [JAVA,
                    # if you need a larger Java heap, use this environment variable
                    '-Xmx' + (os.environ.get('JAVA_HEAP_SIZE') or '1024m'),
            ...
            if proc.returncode != 0:
                ...
                exit_with_error(
                    'closure compiler failed (rc: %d.%s)', proc.returncode, hint)
    ...
    def minify_wasm_js(js_file, wasm_file, expensive_optimizations, minify_whitespace, debug_info, emit_symbol_map):
        ...
        if not Settings.LINKABLE:
            ...
            if expensive_optimizations and not emit_symbol_map:
                js_file = Building.metadce(
                    js_file, wasm_file, minify_whitespace=minify_whitespace, debug_info=debug_info)
                ...
                if minify_whitespace:
                    passes.append('minifyWhitespace')
                logger.debug(
                    'running post-meta-DCE cleanup on shell code: ' + ' '.join(passes))
                js_file = Building.acorn_optimizer(js_file, passes)
                ...
                if Settings.EMITTING_JS and not Settings.AUTODEBUG:
                    js_file = Building.minify_wasm_imports_and_exports(
                        js_file, wasm_file, minify_whitespace=minify_whitespace, minify_exports=Settings.DECLARE_ASM_MODULE_EXPORTS, debug_info=debug_info)
        return js_file
    ...
    def metadce(js_file, wasm_file, minify_whitespace, debug_info):
        ...
        extra_info = '{ "exports": [' + ','.join(
            map(lambda x: '["' + x + '","' + x + '"]', Settings.MODULE_EXPORTS)) + ']}'
        txt = Building.acorn_optimizer(
            js_file, ['emitDCEGraph', 'noPrint'], return_output=True, extra_info=extra_info)
        ...
        for item in graph:
            if 'import' in item:
                import_name_map[item['name']
                                ] = 'emcc$import$' + item['import'][1]
        temp = temp_files.get('.txt').name
        txt = json.dumps(graph)
        with open(temp, 'w') as f:
            f.write(txt)
        # run wasm-metadce
        cmd = [os.path.join(Building.get_binaryen_bin(), 'wasm-metadce'),
               '--graph-file=' + temp, wasm_file, '-o', wasm_file]
        if debug_info:
            cmd += ['-g']
    ...
    def minify_wasm_imports_and_exports(js_file, wasm_file, minify_whitespace, minify_exports, debug_info):
        ...
        # run the pass
        cmd = [os.path.join(Building.get_binaryen_bin(), 'wasm-opt'),
               '--minify-imports-and-exports' if minify_exports else '--minify-imports', wasm_file, '-o', wasm_file]
        cmd += Building.get_binaryen_feature_flags()
        if debug_info:
            cmd.append('-g')
    ...
    def wasm2js(js_file, wasm_file, opt_level, minify_whitespace, use_closure_compiler, debug_info):
        ...
        finds = re.findall(
            r'''[\w\d_$]+\[['"]__wasm2jsInstantiate__['"]\]''', all_js)
        if not finds:
    ...
    def is_ar(filename):
        try:
            ...
            logger.debug(
                'Building.is_ar failed to test whether file \'%s\' is a llvm archive file! Failed on exception: %s' % (filename, e))
            return False
    ...
    def path_to_system_js_libraries(library_name):
        ...
        if library_name in js_system_libraries:
            if len(js_system_libraries[library_name]):
                library_files += js_system_libraries[library_name] if isinstance(
                    js_system_libraries[library_name], list) else [js_system_libraries[library_name]]
            ...
            if Settings.ERROR_ON_MISSING_LIBRARIES:
                exit_with_error(
                    'emcc: cannot find library "%s" (`-s ERROR_ON_MISSING_LIBRARIES=0` to disable this error)', library_name)
            else:
...
function jsCall_%s(index%s) {
    ...
    def make_invoke(sig, named=True):
      ...
      '''
            ...
            # TODO: do this in extcall, jscall?
...
class WebAssembly(object):
    ...
    def add_emscripten_metadata(js_file, wasm_file):
        ...
        logger.debug('creating wasm emscripten metadata section with mem size %d, table size %d' % (
            mem_size, table_size,))
    ...
    def make_shared_library(js_file, wasm_file, needed_dynlibs):
        ...
        logger.debug('creating wasm dynamic library with mem size %d, table size %d, align %d' % (
            mem_size, table_size, mem_align))
...
def read_and_preprocess(filename, expand_macros=False):
    ...
    run_js(path_from_root('tools/preprocessor.js'), NODE_JS,
           args, True, stdout=open(stdout, 'w'), cwd=path)
...
def make_fetch_worker(source_file, output_file):
    ...
    funcs_to_import = ['alignUp', '_emscripten_get_heap_size', '_emscripten_resize_heap', 'stringToUTF8', 'UTF8ToString', 'UTF8ArrayToString',
                       'intArrayFromString', 'lengthBytesUTF8', 'stringToUTF8Array', '_emscripten_is_main_runtime_thread', '_emscripten_futex_wait']
    asm_funcs_to_import = ['_malloc', '_free', '_sbrk', '___pthread_mutex_lock',
                           '___pthread_mutex_unlock', '_pthread_mutexattr_init', '_pthread_mutex_init']
...
'''
    ...
    for func in funcs_to_import + asm_funcs_to_import:
        loc = src.find('function ' + func +
                       '(', asm_start if func in asm_funcs_to_import else 0)
        if loc == -1:
    ...
    fetch_worker_src = function_prologue + '\n' + \
        read_and_preprocess(path_from_root(
            'src', 'fetch-worker.js'), expand_macros=True)
