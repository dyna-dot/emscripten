...
C_ENDINGS = ('.c', '.C', '.i')
CXX_ENDINGS = ('.cpp', '.cxx', '.cc', '.c++',
               '.CPP', '.CXX', '.CC', '.C++', '.ii')
OBJC_ENDINGS = ('.m', '.mi')
OBJCXX_ENDINGS = ('.mm', '.mii')
SPECIAL_ENDINGLESS_FILENAMES = ('/dev/null',)

SOURCE_ENDINGS = C_ENDINGS + CXX_ENDINGS + OBJC_ENDINGS + \
    OBJCXX_ENDINGS + SPECIAL_ENDINGLESS_FILENAMES
# consider the special endingless filenames like /dev/null to be C
C_ENDINGS = C_ENDINGS + SPECIAL_ENDINGLESS_FILENAMES

JS_CONTAINING_ENDINGS = ('.js', '.mjs', '.html')
BITCODE_ENDINGS = ('.bc', '.o', '.obj', '.lo')
# Windows .dll suffix is not included in this list, since those are never linked to directly on the command line.
...
DEFERRED_RESPONSE_FILES = ('EMTERPRETIFY_BLACKLIST',
                           'EMTERPRETIFY_WHITELIST', 'EMTERPRETIFY_SYNCLIST')
...
def save_intermediate(name, suffix='js'):
    if not DEBUG:
        return
    name = os.path.join(shared.get_emscripten_temp_dir(),
                        'emcc-%d-%s.%s' % (Intermediate.counter, name, suffix))
    if isinstance(final, list):
        logger.debug(
            '(not saving intermediate %s because deferring linking)' % name)
        return
...
def save_intermediate_with_wasm(name, wasm_binary):
    ...
    name = os.path.join(shared.get_emscripten_temp_dir(),
                        'emcc-%d-%s.wasm' % (Intermediate.counter - 1, name))
...
def log_time(name):
    ...
    if DEBUG:
        now = time.time()
        logger.debug('emcc step "%s" took %.2f seconds',
                     name, now - TimeLogger.last)
        TimeLogger.update()
...
class EmccOptions(object):
    def __init__(self):
        ...
        self.llvm_opts = None
        self.llvm_lto = None
        # Enforce a consistent C++ standard when compiling .cpp files, if user does not specify one on the cmdline.
        self.default_cxx_std = '-std=c++03'
...
class JSOptimizer(object):
            ...
            if DEBUG != 2 or len(passes) < 2:
                ...
                for p in passes:
                    if len(curr) == 0:
                        ...
                        native = js_optimizer.use_native(
                            p, source_map=use_source_map(self))
                        last_native = js_optimizer.use_native(
                            curr[-1], source_map=use_source_map(self))
                ...
                if len(curr):
                    chunks.append(curr)
                if len(chunks) == 1:
                    self.run_passes(chunks[0], title,
                                    just_split=False, just_concat=False)
                else:
    ...
    def run_passes(self, passes, title, just_split, just_concat):
        ...
        save_intermediate(
            title, suffix='js' if 'emitJSON' not in passes else 'json')
...
def apply_settings(changes):
    ...
    for change in changes:
        ...
        try:
            ...
            exit_with_error(
                'a problem occured in evaluating the content after a "-s", specifically "%s": %s', change, str(e))
...
''' % (open(shared.path_from_root('site', 'build', 'text', 'docs', 'tools_reference', 'emcc.txt')).read()))
    ...
    if '--version' in args:
        try:
            revision = run_process(['git', 'show'], stdout=PIPE, stderr=PIPE,
                                   cwd=shared.path_from_root()).stdout.splitlines()[0]
        except:
...
There is NO warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
  ''' % (shared.EMSCRIPTEN_VERSION, revision))
    ...
    if len(args) == 1 and args[0] == '-v':  # -v with no inputs
        ...
        print('emcc (Emscripten gcc/clang-like replacement + linker emulating GNU ld) %s' %
              shared.EMSCRIPTEN_VERSION, file=sys.stderr)
    ...
    # gcc's doc states "Print the compiler version [...] and don't do anything else."
    ...
    if '--cflags' in args:
        ...
        with misc_temp_files.get_file(suffix='.o') as temp_target:
            ...
            err = run_process([shared.PYTHON, sys.argv[0]] + args + [shared.path_from_root(
                'tests', input_file), '-c', '-o', temp_target], stderr=PIPE, env=debug_env).stderr
            ...
            parts = [x for x in parts if x != '-c' and x !=
                     '-o' and input_file not in x and temp_target not in x and '-emit-llvm' not in x]
    ...
    def is_minus_s_for_emcc(args, i):
        ...
        logger.debug(
            'treating -s as linker option and not as -s OPT=VALUE for js compilation')
        return False

    # If this is a configure-type thing, do not compile to JavaScript, instead use clang
    # to compile to a native binary (using our headers, so things make sense later)
    CONFIGURE_CONFIG = (os.environ.get('EMMAKEN_JUST_CONFIGURE')
                        or 'conftest.c' in args) and not os.environ.get('EMMAKEN_JUST_CONFIGURE_RECURSE')
    # or 'CMakeCCompilerId' in ' '.join(args)
    ...
    if CONFIGURE_CONFIG or CMAKE_CONFIG:
        ...
        for arg in args:
            if arg.endswith(SOURCE_ENDINGS):
                ...
                if debug_configure:
                    open(tempout, 'a').write(
                        '(compiling .s assembly, must use clang\n')
                if use_js == 1:
                    use_js = 0
        ...
        if src:
            if 'fopen' in src and '"w"' in src:
                ...
                if debug_configure:
                    open(tempout, 'a').write(
                        'Forcing clang since uses fopen to write\n')
        ...
        if debug_configure:
            open(tempout, 'a').write(
                'emcc, just configuring: ' + ' '.join(cmd) + '\n\n')
        ...
        if not target.endswith(BITCODE_ENDINGS):
            ...
            if os.path.sep not in full_node:
                ...
                full_node = '/usr/bin/' + full_node
            open(target, 'w').write(
                '#!' + full_node + '\n' + src)  # add shebang
            try:
                os.chmod(target, stat.S_IMODE(os.stat(target).st_mode)
                         | stat.S_IXUSR)  # make executable
            except:
    ...
    if specified_target:
        ...
        if dirname and not os.path.isdir(dirname):
            exit_with_error(
                "specified output file (%s) is in a directory that does not exist" % target)
    else:
        target = 'a.out.js'

    shared.Settings.TARGET_BASENAME = target_basename = unsuffixed_basename(
        target)
    ...
    with ToolchainProfiler.profile_block('parse arguments and setup'):
        ...
        if options.emrun:
            options.pre_js += open(shared.path_from_root('src',
                                                         'emrun_prejs.js')).read() + '\n'
            options.post_js += open(shared.path_from_root('src',
                                                          'emrun_postjs.js')).read() + '\n'
        ...
        if options.memory_profiler:
            options.post_js += open(shared.path_from_root('src',
                                                          'memoryprofiler.js')).read() + '\n'

        if options.thread_profiler:
            options.post_js += open(shared.path_from_root('src',
                                                          'threadprofiler.js')).read() + '\n'
        ...
        if options.js_transform and use_source_map(options):
            logger.warning(
                'disabling source maps because a js transform is being done')
            ...
            if newargs[i] == '-s':
                if is_minus_s_for_emcc(newargs, i):
                    ...
                    if key == 'WASM_BACKEND=1':
                        exit_with_error(
                            'do not set -s WASM_BACKEND, instead set EMCC_WASM_BACKEND=1 in the environment')
        ...
        for i in range(len(newargs)):
            ...
            if not arg.startswith('-'):
                if not os.path.exists(arg):
                    exit_with_error(
                        '%s: No such file or directory ("%s" was expected to be an input file, based on the commandline arguments provided)', arg, arg)

                file_suffix = get_file_suffix(arg)
                # we already removed -o <target>, so all these should be inputs
                if file_suffix in SOURCE_ENDINGS + BITCODE_ENDINGS + DYNAMICLIB_ENDINGS + ASSEMBLY_ENDINGS + HEADER_ENDINGS or shared.Building.is_ar(arg):
                    newargs[i] = ''
                    if file_suffix.endswith(SOURCE_ENDINGS):
                        ...
                        if not shared.Settings.WASM_BACKEND:
                            exit_with_error(
                                'fastcomp is not compatible with wasm object files:' + arg)
                        input_files.append((i, arg))
                    ...
                    if not shared.Building.is_ar(arg):
                        if shared.Building.is_bitcode(arg):
                            message = arg + ': File has a suffix of a static library ' + \
                                str(STATICLIB_ENDINGS) + ', but instead is an LLVM bitcode file! When linking LLVM bitcode files, use one of the suffixes ' + str(BITCODE_ENDINGS)
                        else:
                    ...
                    if has_fixed_language_mode:
                        ...
                        exit_with_error(
                            arg + ": Input file has an unknown suffix, don't know what to do with it!")
                ...
                for flag_index, flag in enumerate(link_flags_to_add):
                    if flag.startswith('-l'):
                        ...
                        link_flags.append(
                            (i + float(flag_index) / len(link_flags_to_add), flag))
        ...
        if has_dash_c:
            ...
            final_suffix = '.o'
        if '-E' in newargs:
            # not bitcode, not js; but just result from preprocessing stage of the input file
            final_suffix = '.eout'
        ...
        # might not be used, but if it is, this is the name
        asm_target = unsuffixed(js_target) + '.asm.js'
        wasm_text_target = asm_target.replace(
            '.asm.js', '.wast')  # ditto, might not be used
        wasm_binary_target = asm_target.replace(
            '.asm.js', '.wasm')  # ditto, might not be used
        ...
        if final_suffix == '.html' and not options.separate_asm and 'PRECISE_F32=2' in settings_changes:
            ...
            logger.warning(
                'forcing separate asm output (--separate-asm), because -s PRECISE_F32=2 was passed.')
        if options.separate_asm:
            shared.Settings.SEPARATE_ASM = shared.JS.get_subresource_location(
                asm_target)
        ...
        error_on_missing_libraries_cmdline = get_last_setting_change(
            'ERROR_ON_MISSING_LIBRARIES')
        if error_on_missing_libraries_cmdline:
            shared.Settings.ERROR_ON_MISSING_LIBRARIES = int(
                error_on_missing_libraries_cmdline[len('ERROR_ON_MISSING_LIBRARIES='):])
            ...
            def check(input_file):
                if get_file_suffix(input_file) in DYNAMICLIB_ENDINGS:
                    if not options.ignore_dynamic_linking:
                        logger.warning(
                            'ignoring dynamic library %s because not compiling to JS or HTML, remember to link it when compiling to JS or HTML at the end', os.path.basename(input_file))
                    return False
                else:
                    return True
        ...
        if len(input_files) == 0:
            exit_with_error('no input files\nnote that input files without a known suffix are ignored, make sure your input files end with one of: ' +
                            str(SOURCE_ENDINGS + BITCODE_ENDINGS + DYNAMICLIB_ENDINGS + STATICLIB_ENDINGS + ASSEMBLY_ENDINGS + HEADER_ENDINGS))
        ...
        shared.Settings.apply_opt_level(
            opt_level=options.opt_level, shrink_level=options.shrink_level, noisy=True)
        ...
        if options.bind:
            ...
            if final_suffix in JS_CONTAINING_ENDINGS:
                input_files.append((next_arg_index, shared.path_from_root(
        ...
        if shared.Settings.WASM_OBJECT_FILES and not shared.Settings.WASM_BACKEND:
            if 'WASM_OBJECT_FILES=1' in settings_changes:
                exit_with_error(
                    'WASM_OBJECT_FILES can only be used with wasm backend')
        ...
        if options.use_closure_compiler:
            ...
            if not shared.check_closure_compiler():
                exit_with_error(
                    'fatal: closure compiler is not configured correctly')
            if options.use_closure_compiler == 2 and shared.Settings.ASM_JS == 1:
                shared.WarningManager.warn(
                    'ALMOST_ASM', 'not all asm.js optimizations are possible with --closure 2, disabling those - your code will be run more slowly')
        ...
        if shared.Settings.USE_PTHREADS:
            ...
            shared.Settings.EXPORTED_RUNTIME_METHODS += [
                'establishStackSpace', 'dynCall_ii']
        ...
        if shared.Settings.STB_IMAGE and final_suffix in JS_CONTAINING_ENDINGS:
            input_files.append(
                (next_arg_index, shared.path_from_root('third_party', 'stb_image.c')))
            next_arg_index += 1
            shared.Settings.EXPORTED_FUNCTIONS += ['_stbi_load',
                                                   '_stbi_load_from_memory', '_stbi_image_free']
        ...
        if shared.Settings.ASMFS and final_suffix in JS_CONTAINING_ENDINGS:
            ...
            options.js_libraries.append(
                shared.path_from_root('src', 'library_asmfs.js'))
        ...
        if shared.Settings.FETCH and final_suffix in JS_CONTAINING_ENDINGS:
            ...
            next_arg_index += 1
            options.js_libraries.append(
                shared.path_from_root('src', 'library_fetch.js'))
            if shared.Settings.USE_PTHREADS:
                shared.Settings.FETCH_WORKER_FILE = unsuffixed(
                    os.path.basename(target)) + '.fetch.js'
        ...
        if shared.Settings.RELOCATABLE and not shared.Settings.DYNAMIC_EXECUTION:
            exit_with_error(
                'cannot have both DYNAMIC_EXECUTION=0 and RELOCATABLE enabled at the same time, since RELOCATABLE needs to eval()')
        ...
        if shared.Settings.RELOCATABLE:
            ...
            if 'EMULATED_FUNCTION_POINTERS' not in settings_key_changes and not shared.Settings.WASM_BACKEND:
                # by default, use optimized function pointer emulation
                shared.Settings.EMULATED_FUNCTION_POINTERS = 2
            shared.Settings.ERROR_ON_UNDEFINED_SYMBOLS = 0
        ...
        if shared.Settings.EMTERPRETIFY:
            ...
            if options.use_closure_compiler == 2:
                exit_with_error(
                    'EMTERPRETIFY requires valid asm.js, and is incompatible with closure 2 which disables that')
            assert not use_source_map(
                options), 'EMTERPRETIFY is not compatible with source maps (maps are not useful in emterpreted code, and splitting out non-emterpreted source maps is not yet implemented)'
        ...
        if options.use_preload_plugins or len(options.preload_files) or len(options.embed_files):
            if shared.Settings.NODERAWFS:
                exit_with_error(
                    '--preload-file and --embed-file cannot be used with NODERAWFS which disables virtual filesystem')
        ...
        if shared.Settings.FILESYSTEM and not shared.Settings.ONLY_MY_CODE:
            if shared.Settings.SUPPORT_ERRNO:
                # so FS can report errno back to C
        ...
        if shared.Settings.USE_PTHREADS:
            ...
            if shared.Settings.ALLOW_MEMORY_GROWTH:
                if not shared.Settings.WASM:
                    exit_with_error(
                        'Memory growth is not supported with pthreads without wasm')
                else:
                    logging.warning(
            ...
            options.js_libraries.append(
                shared.path_from_root('src', 'library_pthread.js'))
            newargs.append('-D__EMSCRIPTEN_PTHREADS__=1')
            # set location of worker.js
            shared.Settings.PTHREAD_WORKER_FILE = unsuffixed(
                os.path.basename(target)) + '.worker.js'
        else:
            options.js_libraries.append(
                shared.path_from_root('src', 'library_pthread_stub.js'))
        ...
        if shared.Settings.USE_PTHREADS:
            ...
            if shared.Settings.LINKABLE:
                exit_with_error(
                    '-s LINKABLE=1 is not supported with -s USE_PTHREADS>0!')
            if shared.Settings.SIDE_MODULE:
                exit_with_error(
                    '-s SIDE_MODULE=1 is not supported with -s USE_PTHREADS>0!')
            if shared.Settings.MAIN_MODULE:
                exit_with_error(
                    '-s MAIN_MODULE=1 is not supported with -s USE_PTHREADS>0!')
            if shared.Settings.EMTERPRETIFY:
                exit_with_error(
                    '-s EMTERPRETIFY=1 is not supported with -s USE_PTHREADS>0!')
        ...
        if shared.Settings.WASM:
            if shared.Settings.TOTAL_MEMORY % 65536 != 0:
                exit_with_error(
                    'For wasm, TOTAL_MEMORY must be a multiple of 64KB, was ' + str(shared.Settings.TOTAL_MEMORY))
        else:
            if shared.Settings.TOTAL_MEMORY < 16 * 1024 * 1024:
                exit_with_error(
                    'TOTAL_MEMORY must be at least 16MB, was ' + str(shared.Settings.TOTAL_MEMORY))
            if shared.Settings.TOTAL_MEMORY % (16 * 1024 * 1024) != 0:
                exit_with_error(
                    'For asm.js, TOTAL_MEMORY must be a multiple of 16MB, was ' + str(shared.Settings.TOTAL_MEMORY))
        if shared.Settings.TOTAL_MEMORY < shared.Settings.TOTAL_STACK:
            exit_with_error('TOTAL_MEMORY must be larger than TOTAL_STACK, was ' + str(
                shared.Settings.TOTAL_MEMORY) + ' (TOTAL_STACK=' + str(shared.Settings.TOTAL_STACK) + ')')
        if shared.Settings.WASM_MEM_MAX != -1 and shared.Settings.WASM_MEM_MAX % 65536 != 0:
            exit_with_error(
                'WASM_MEM_MAX must be a multiple of 64KB, was ' + str(shared.Settings.WASM_MEM_MAX))
        if shared.Settings.USE_PTHREADS and shared.Settings.WASM and shared.Settings.ALLOW_MEMORY_GROWTH and shared.Settings.WASM_MEM_MAX == -1:
            exit_with_error(
                'If pthreads and memory growth are enabled, WASM_MEM_MAX must be set')
        ...
        if shared.Settings.MODULARIZE and not shared.Settings.DECLARE_ASM_MODULE_EXPORTS:
            ...
            logger.warning(
                'Enabling -s DECLARE_ASM_MODULE_EXPORTS=1, since MODULARIZE currently requires declaring asm.js/wasm module exports in full')
        ...
        if not shared.Settings.SEPARATE_ASM_MODULE_NAME:
            ...
            exit_with_error(
                '-s SEPARATE_ASM_MODULE_NAME option only applies to when targeting asm.js, not with WebAssembly!')

        if shared.Settings.MINIMAL_RUNTIME:
            # Minimal runtime uses a different default shell file
            if options.shell_path == shared.path_from_root('src', 'shell.html'):
                options.shell_path = shared.path_from_root(
        ...
        if shared.Settings.MODULARIZE and not shared.Settings.MODULARIZE_INSTANCE and shared.Settings.EXPORT_NAME == 'Module' and final_suffix == '.html' and \
           (options.shell_path == shared.path_from_root('src', 'shell.html') or options.shell_path == shared.path_from_root('src', 'shell_minimal.html')):
            exit_with_error('Due to collision in variable name "Module", the shell file "' + options.shell_path +
                            '" is not compatible with build options "-s MODULARIZE=1 -s EXPORT_NAME=Module". Either provide your own shell file, change the name of the export to something else to avoid the name collision. (see https://github.com/emscripten-core/emscripten/issues/7950 for details)')
        ...
        if shared.Settings.WASM:
            if shared.Settings.SINGLE_FILE:
                ...
                shared.Settings.WASM_TEXT_FILE = shared.JS.escape_for_js_string(
                    os.path.basename(wasm_text_target))
                shared.Settings.WASM_BINARY_FILE = shared.JS.escape_for_js_string(
                    os.path.basename(wasm_binary_target))
                shared.Settings.ASMJS_CODE_FILE = shared.JS.escape_for_js_string(
                    os.path.basename(asm_target))
            # when targeting wasm, we use a wasm Memory, but that is not compatible with asm.js opts
            shared.Settings.ASM_JS = 2
            ...
            if shared.Settings.ELIMINATE_DUPLICATE_FUNCTIONS:
                logger.warning(
                    'for wasm there is no need to set ELIMINATE_DUPLICATE_FUNCTIONS, the binaryen optimizer does it automatically')
            ...
            if options.js_opts and not options.force_js_opts:
                ...
                logger.debug(
                    'asm.js opts not forced by user or an option that depends them, and we do not intend to run the asm.js, so disabling and leaving opts to the binaryen optimizer')
            if options.use_closure_compiler == 2 and not shared.Settings.WASM2JS:
                exit_with_error(
                    'closure compiler mode 2 assumes the code is asm.js, so not meaningful for wasm')
            if any(s.startswith('MEM_INIT_METHOD=') for s in settings_changes):
                exit_with_error(
                    'MEM_INIT_METHOD is not supported in wasm. Memory will be embedded in the wasm binary if threads are not used, and included in a separate file if threads are used.')
            ...
            if not shared.Settings.MINIMAL_RUNTIME:
                if shared.Settings.WASM_ASYNC_COMPILATION == 1:
                    ...
                    if 'WASM_ASYNC_COMPILATION=1' in settings_changes:
                        logger.warning(
                            'WASM_ASYNC_COMPILATION requested, but disabled because of user options. ' + warning)
                    elif 'WASM_ASYNC_COMPILATION=0' not in settings_changes:
                        logger.warning(
                            'WASM_ASYNC_COMPILATION disabled due to user options. ' + warning)
            ...
            if not shared.Settings.DECLARE_ASM_MODULE_EXPORTS:
                ...
                if shared.Settings.SWAPPABLE_ASM_MODULE == 1:
                    shared.Settings.DECLARE_ASM_MODULE_EXPORTS = 1
                    logger.warning(
                        'Enabling -s DECLARE_ASM_MODULE_EXPORTS=1 since -s SWAPPABLE_ASM_MODULE=1 is used')

            # wasm side modules have suffix .wasm
            if shared.Settings.SIDE_MODULE and target.endswith('.js'):
                logger.warning(
                    'output suffix .js requested, but wasm side modules are just wasm files; emitting only a .wasm, no .js')

            if options.separate_asm:
                exit_with_error(
                    'cannot --separate-asm when emitting wasm, since not emitting asm.js')
            ...
            if shared.Settings.WASM_BACKEND:
                ...
                if 'BINARYEN_PASSES' not in settings_key_changes:
                    ...
                    if options.opt_level > 0 or options.shrink_level > 0:
                        passes += [shared.Building.opt_level_to_str(
                            options.opt_level, options.shrink_level)]
        ...
        if shared.Settings.WASM2JS:
            if not shared.Settings.WASM_BACKEND:
                exit_with_error(
                    'wasm2js is only available in the upstream wasm backend path')
            logger.warning(
                'emcc: JS support in the upstream LLVM+wasm2js path is very experimental currently (best to use fastcomp for asm.js for now)')
        ...
        if shared.Settings.MINIMAL_RUNTIME:
            if shared.Settings.ALLOW_MEMORY_GROWTH:
                logging.warning(
                    '-s ALLOW_MEMORY_GROWTH=1 is not yet supported with -s MINIMAL_RUNTIME=1')

            if shared.Settings.EMTERPRETIFY:
                exit_with_error(
                    '-s EMTERPRETIFY=1 is not supported with -s MINIMAL_RUNTIME=1')

            if shared.Settings.USE_PTHREADS:
                exit_with_error(
                    '-s USE_PTHREADS=1 is not yet supported with -s MINIMAL_RUNTIME=1')

            if shared.Settings.PRECISE_F32 == 2:
                exit_with_error(
                    '-s PRECISE_F32=2 is not supported with -s MINIMAL_RUNTIME=1')

            if shared.Settings.SINGLE_FILE:
                exit_with_error(
                    '-s SINGLE_FILE=1 is not supported with -s MINIMAL_RUNTIME=1')
        ...
        if shared.Settings.ALLOW_MEMORY_GROWTH and shared.Settings.ASM_JS == 1:
            ...
            if not shared.Settings.WASM:
                ...
                # memory growth does not validate as asm.js http://discourse.wicg.io/t/request-for-comments-switching-resizing-heaps-in-asm-js/641/23
                shared.Settings.ASM_JS = 2
        ...
        if shared.Settings.CYBERDWARF:
            ...
            options.js_libraries.append(
                shared.path_from_root('src', 'library_cyberdwarf.js'))
            options.js_libraries.append(shared.path_from_root(
        ...
        if not shared.Settings.LEGALIZE_JS_FFI:
            assert shared.Building.is_wasm_only(
            ), 'LEGALIZE_JS_FFI incompatible with RUNNING_JS_OPTS.'
    ...
    try:
        with ToolchainProfiler.profile_block('bitcodeize inputs'):
            ...
            if has_header_inputs:
                ...
                for header in headers:
                    assert header.endswith(
                        HEADER_ENDINGS), 'if you have one header input, we assume you want to precompile headers, and cannot have source files or other inputs as well: ' + str(headers) + ' : ' + header
                args = newargs + headers
                if specified_target:
                    args += ['-o', specified_target]
                args = system_libs.process_args(args, shared.Settings)
                logger.debug("running (for precompiled headers): " +
                             clang_compiler + ' ' + ' '.join(args))
            ...
            if '-E' in newargs or '-M' in newargs or '-MM' in newargs:
                ...
                logger.debug(
                    ('just preprocessor ' if '-E' in newargs else 'just dependencies: ') + ' '.join(cmd))
                return run_process(cmd, check=False).returncode
            ...
            def compile_source_file(i, input_file):
                ...
                # NOTE: Printing this line here in this specific format is important, it is parsed to implement the "emcc --cflags" command
                logger.debug(
                    "running: " + ' '.join(shared.Building.doublequote_spaces(args)))
                ...
                if file_ending.endswith(SOURCE_ENDINGS):
                    ...
                    if file_ending.endswith(BITCODE_ENDINGS):
                        ...
                        if not LEAVE_INPUTS_RAW:
                            logger.debug(
                                'assembling assembly file: ' + input_file)
                            temp_file = in_temp(unsuffixed(
                                uniquename(input_file)) + '.o')
                        ...
                        if has_fixed_language_mode:
                            ...
                            exit_with_error(
                                input_file + ': Unknown file suffix when compiling to LLVM bitcode!')
                ...
                if optimizing(options.llvm_opts):
                    for pos, (_, input_file) in enumerate(input_files):
                        ...
                        if file_ending.endswith(SOURCE_ENDINGS):
                            ...
                            new_temp_file = in_temp(
                                unsuffixed(uniquename(temp_file)) + '.o')
                            ...
                            opts = options.llvm_opts + \
                                ['-lower-non-em-intrinsics']
                            shared.Building.llvm_opt(
                                temp_file, opts, new_temp_file)
                            temp_files[pos] = (
                                temp_files[pos][0], new_temp_file)
            ...
            if stop_at_bitcode:
                if not specified_target:
                    ...
                    for tempf, inputf in zip(temp_files, input_files):
                        safe_move(tempf[1], unsuffixed_basename(
                            inputf[1]) + final_suffix)
                else:
                    if len(input_files) == 1:
                        input_file = input_files[0][1]
                        temp_file = temp_files[0][1]
                        bitcode_target = specified_target if specified_target else unsuffixed_basename(
                            input_file) + final_suffix
                        ...
                        if os.path.exists(temp_output_base + '.d'):
                            ...
                            deps = deps.replace(
                                temp_output_base + options.default_object_extension, specified_target)
                            with open(os.path.join(os.path.dirname(specified_target), os.path.basename(unsuffixed(input_file) + '.d')), "w") as out_dep:
                                out_dep.write(deps)
                    else:
                        assert len(original_input_files) == 1 or not has_dash_c, 'fatal error: cannot specify -o with -c with multiple files' + str(
                            args) + ':' + str(original_input_files)
                        # We have a specified target (-o <target>), which is not JavaScript or HTML, and
                        # we have multiple files: Link them
                        logger.debug(
                            'link: ' + str(linker_inputs) + specified_target)
                        shared.Building.link_to_object(
                            linker_inputs, specified_target)
        ...
        with ToolchainProfiler.profile_block('calculate system libraries'):
            ...
            if not LEAVE_INPUTS_RAW and \
               ...
               not shared.Settings.SIDE_MODULE:  # shared libraries/side modules link no C libraries, need them in parent
                ...
                extra_files_to_link += system_libs.calculate([f for _, f in sorted(
                    temp_files)] + extra_files_to_link, in_temp, stdout_=None, stderr_=None, forced=forced_stdlibs)
        ...
        shared.Settings.EXPORTED_FUNCTIONS = dedup_list(
            shared.Settings.EXPORTED_FUNCTIONS)
        ...
        with ToolchainProfiler.profile_block('link'):
            ...
            perform_link = len(
                linker_inputs) > 1 or shared.Settings.WASM_BACKEND
            ...
            if perform_link:
                ...
                force_archive_contents = all(t.endswith(
                    STATICLIB_ENDINGS) for _, t in temp_files) or shared.Settings.LINKABLE
                ...
                if shared.Settings.WASM_BACKEND:
                    ...
                    if options.llvm_lto:
                        ...
                        lto_level = 0
                    final = shared.Building.link_lld(
                        linker_inputs, DEFAULT_FINAL, lto_level=lto_level)
                else:
                    final = shared.Building.link(
                        linker_inputs, DEFAULT_FINAL, force_archive_contents=force_archive_contents, temp_files=misc_temp_files, just_calculate=just_calculate)
            else:
        ...
        if not shared.Settings.WASM_BACKEND:
            with ToolchainProfiler.profile_block('post-link'):
                if DEBUG:
                    logger.debug(
                        'saving intermediate processing steps to %s', shared.get_emscripten_temp_dir())
                    if not LEAVE_INPUTS_RAW:
                ...
                if not LEAVE_INPUTS_RAW:
                    ...
                    link_opts = [] if use_source_map(options) or shared.Settings.CYBERDWARF else [
                        '-strip-debug']
                    ...
                    if options.llvm_lto is not None and options.llvm_lto >= 2 and optimizing(options.llvm_opts):
                        ...
                        final = shared.Building.llvm_opt(
                            final, options.llvm_opts, DEFAULT_FINAL)
                    ...
                    if AUTODEBUG:
                        ...
                        link_opts += ['-S']
                        final = shared.Building.llvm_opt(
                            final, link_opts, get_final() + '.link.ll')
                ...
                if AUTODEBUG:
                    ...
                    run_process(
                        [shared.PYTHON, shared.AUTODEBUGGER, final, next])
                    final = next
                    save_intermediate('autodebug', 'll')

                assert not isinstance(
                    final, list), 'we must have linked the final files, if linking was deferred, by this point'
        ...
        with ToolchainProfiler.profile_block('emscript'):
            ...
            if options.js_libraries:
                js_libraries = [os.path.abspath(lib)
                                for lib in options.js_libraries]
            ...
            final = shared.Building.emscripten(
                final, target + '.mem', js_libraries)
        ...
        with ToolchainProfiler.profile_block('source transforms'):
            ...
            if len(options.preload_files) or len(options.embed_files):
                ...
                if shared.Settings.ALLOW_MEMORY_GROWTH and not options.no_heap_copy:
                    logger.info(
                        'Enabling --no-heap-copy because -s ALLOW_MEMORY_GROWTH=1 is being used with file_packager.py (pass --no-heap-copy to suppress this notification)')
                    options.no_heap_copy = True

                logger.debug('setting up files')
                file_args = ['--from-emcc', '--export-name=' +
                ...
                file_code = run_process([shared.PYTHON, shared.FILE_PACKAGER, unsuffixed(
                    target) + '.data'] + file_args, stdout=PIPE).stdout
            ...
            if options.js_transform:
                ...
                shared.check_call(shared.Building.remove_quotes(shlex.split(
                    options.js_transform, posix=posix) + [os.path.abspath(final)]))
        ...
        with ToolchainProfiler.profile_block('memory initializer'):
            ...
            if (not shared.Settings.WASM_BACKEND and (shared.Settings.MEM_INIT_METHOD > 0 or embed_memfile(options))) or \
               (shared.Settings.WASM_BACKEND and not shared.Settings.MEM_INIT_IN_WASM):
                if shared.Settings.MINIMAL_RUNTIME:
                    ...
                    memfile = target.replace(
                        '.html', '.mem').replace('.js', '.mem')
                else:
                    memfile = target + '.mem'
            ...
            if memfile:
                if shared.Settings.WASM_BACKEND:
                    ...
                    src = open(final).read()
                    src = src.replace('var memoryInitializer = null;',
                                      'var memoryInitializer = "%s";' % os.path.basename(memfile))
                    ...
                    def repl(m):
                        ...
                        if DEBUG:
                            ...
                            shared.safe_copy(memfile, os.path.join(
                                shared.get_emscripten_temp_dir(), os.path.basename(memfile)))
                    ...
                    src = re.sub(shared.JS.memory_initializer_pattern,
                                 repl, open(final).read(), count=1)
                    open(final + '.mem.js', 'w').write(src)
                    final += '.mem.js'
                    src = None
                    # simple text substitution preserves comment line number mappings
                    js_transform_tempfiles[-1] = final
                    if os.path.exists(memfile):
                        save_intermediate('meminit')
                        logger.debug(
                            'wrote memory initialization to %s', memfile)
                    else:
            ...
            if shared.Settings.USE_PTHREADS:
                ...
                worker_output = os.path.join(
                    target_dir, shared.Settings.PTHREAD_WORKER_FILE)
                with open(worker_output, 'w') as f:
                    f.write(shared.read_and_preprocess(
                        shared.path_from_root('src', 'worker.js'), expand_macros=True))
            ...
            if shared.Settings.FETCH and shared.Settings.USE_PTHREADS:
                if shared.Settings.WASM_BACKEND:
                    ...
                    shared.make_fetch_worker(
                        final, shared.Settings.FETCH_WORKER_FILE)
        ...
        with ToolchainProfiler.profile_block('js opts'):
            ...
            if options.use_closure_compiler == 2 and not shared.Settings.WASM_BACKEND:
                ...
                final = shared.Building.closure_compiler(
                    final, pretty=options.debug_level >= 1)
        ...
        with ToolchainProfiler.profile_block('final emitting'):
            ...
            if shared.Settings.CYBERDWARF:
                run_process([shared.PYTHON, shared.path_from_root(
                    'tools', 'emdebug_cd_merger.py'), target + '.cd', target + '.symbols'])
            ...
            if shared.Settings.MINIMAL_RUNTIME == 2 and shared.Settings.USE_CLOSURE_COMPILER and options.debug_level == 0:
                ...
                shared.run_process([shared.PYTHON, shared.path_from_root(
                    'tools', 'hacky_postprocess_around_closure_limitations.py'), final])
                # Process .asm.js file
                if not shared.Settings.WASM:
                    shared.run_process([shared.PYTHON, shared.path_from_root(
                        'tools', 'hacky_postprocess_around_closure_limitations.py'), asm_target])
            ...
            for f in generated_text_files_with_native_eols:
                tools.line_endings.convert_line_endings_in_file(
                    f, os.linesep, options.output_eol)
...
def parse_args(newargs):
    ...
    def check_bad_eq(arg):
        if '=' in arg:
            exit_with_error(
                'Invalid parameter (do not use "=" with "--" options)')
    ...
    for i in range(len(newargs)):
        ...
        if newargs[i].startswith('-O'):
            ...
            options.opt_level = validate_arg_level(
                options.requested_level, 3, 'Invalid optimization level: ' + newargs[i], clamp=True)
            ...
            options.debug_level = validate_arg_level(
                requested_level, 4, 'Invalid debug level: ' + newargs[i])
            ...
            options.js_libraries.append(
                shared.path_from_root('src', 'library_trace.js'))
            ...
            newargs[i] = ''
            options.js_libraries.append(
                shared.path_from_root('src', 'embind', 'emval.js'))
            options.js_libraries.append(
                shared.path_from_root('src', 'embind', 'embind.js'))
            if options.default_cxx_std:
            ...
            shared.Cache.erase()
            # this is a good time for a sanity check
            shared.check_sanity(force=True)
            should_exit = True
        elif newargs[i] == '--clear-ports':
            logger.info(
                'clearing ports and cache as requested by --clear-ports')
            ...
            if newargs[i + 1].lower() == 'windows':
                ...
                exit_with_error('Invalid value "' +
                                newargs[i + 1] + '" to --output_eol!')
            newargs[i] = ''
            newargs[i + 1] = ''
            ...
            if os.path.exists(path):
                exit_with_error('File ' + optarg +
                                ' passed to --generate-config already exists!')
            else:
...
def separate_asm_js(final, asm_target):
    ...
    shared.check_call([shared.PYTHON, shared.path_from_root('tools', 'separate_asm.py'),
                       final, asm_target, final, shared.Settings.SEPARATE_ASM_MODULE_NAME])

    # extra only-my-code logic
    if shared.Settings.ONLY_MY_CODE:
        temp = asm_target + '.only.js'
        jsrun.run_js_tool(shared.path_from_root('tools', 'js-optimizer.js'), shared.NODE_JS,
                          jsargs=[asm_target, 'eliminateDeadGlobals', 'last', 'asm'], stdout=open(temp, 'w'))
...
def do_binaryen(target, asm_target, options, memfile, wasm_binary_target,
    ...
    if not shared.Settings.WASM_BACKEND:
        if DEBUG:
            ...
            shared.safe_copy(asm_target, os.path.join(
                shared.get_emscripten_temp_dir(), os.path.basename(asm_target)))
        cmd = [os.path.join(binaryen_bin, 'asm2wasm'), asm_target,
               '--total-memory=' + str(shared.Settings.TOTAL_MEMORY)]
        if shared.Settings.BINARYEN_TRAP_MODE in ('js', 'clamp', 'allow'):
            cmd += ['--trap-mode=' + shared.Settings.BINARYEN_TRAP_MODE]
        else:
            exit_with_error('invalid BINARYEN_TRAP_MODE value: ' +
                            shared.Settings.BINARYEN_TRAP_MODE + ' (should be js/clamp/allow)')
        ...
        if options.opt_level > 0:
            cmd.append(shared.Building.opt_level_to_str(
                options.opt_level, options.shrink_level))
        ...
        if shared.Building.is_wasm_only():
            # this asm.js is code not intended to run as asm.js, it is only ever going to be wasm, an can contain special fastcomp-wasm support
            cmd += ['--wasm-only']
        if shared.Settings.USE_PTHREADS:
            cmd += ['--enable-threads']
        if debug_info:
        ...
        if not target_binary:
            cmd = [os.path.join(binaryen_bin, 'wasm-as'), wasm_text_target, '-o',
                   wasm_binary_target, '--all-features', '--disable-bulk-memory']
            if debug_info:
                cmd += ['-g']
                if use_source_map(options):
                    cmd += ['--source-map=' + wasm_source_map_target]
                    cmd += ['--source-map-url=' + options.source_map_base +
                            os.path.basename(wasm_binary_target) + '.map']
        ...
        if import_mem_init:
            ...
            if DEBUG:
                safe_move(memfile, os.path.join(
                    shared.get_emscripten_temp_dir(), os.path.basename(memfile)))
            else:
                os.unlink(memfile)
        log_time('asm2wasm')
    if shared.Settings.BINARYEN_PASSES:
        if DEBUG:
            shared.safe_copy(wasm_binary_target, os.path.join(
                shared.get_emscripten_temp_dir(), os.path.basename(wasm_binary_target) + '.pre-byn'))
        # BINARYEN_PASSES is comma-separated, and we support both '-'-prefixed and unprefixed pass names
        passes = [('--' + p) if p[0] !=
                  '-' else p for p in shared.Settings.BINARYEN_PASSES.split(',')]
        cmd = [os.path.join(binaryen_bin, 'wasm-opt'),
               wasm_binary_target, '-o', wasm_binary_target] + passes
        cmd += shared.Building.get_binaryen_feature_flags()
        if debug_info:
        ...
        if use_source_map(options):
            ...
            cmd += ['--output-source-map-url=' + options.source_map_base +
                    os.path.basename(wasm_binary_target) + '.map']
            if DEBUG:
                shared.safe_copy(wasm_source_map_target, os.path.join(
                    shared.get_emscripten_temp_dir(), os.path.basename(wasm_source_map_target) + '.pre-byn'))
    ...
    if shared.Settings.BINARYEN_SCRIPTS:
        ...
        for script in shared.Settings.BINARYEN_SCRIPTS.split(','):
            ...
            shared.check_call([shared.PYTHON, os.path.join(
                binaryen_scripts, script), final, wasm_text_target], env=script_env)
    if shared.Settings.EVAL_CTORS:
        if DEBUG:
            save_intermediate_with_wasm('pre-eval-ctors', wasm_binary_target)
        shared.Building.eval_ctors(
            final, wasm_binary_target, binaryen_bin, debug_info=debug_info)

    # after generating the wasm, do some final operations
    if shared.Settings.SIDE_MODULE and not shared.Settings.WASM_BACKEND:
        wso = shared.WebAssembly.make_shared_library(
            final, wasm_binary_target, shared.Settings.RUNTIME_LINKED_LIBS)
        ...
        if not DEBUG:
            # we don't need the asm.js, it can just confuse
            os.unlink(asm_target)
    ...
    def run_closure_compiler(final):
        final = shared.Building.closure_compiler(
            final, pretty=not optimizer.minify_whitespace)
        ...
        for target, replacement_string, should_embed in (
            ...
            if should_embed and os.path.isfile(target):
                js = js.replace(replacement_string,
                                shared.JS.get_subresource_location(target))
            else:
                js = js.replace(replacement_string, '')
...
def modularize():
    global final
    logger.debug('Modularizing, assigning to var ' +
                 shared.Settings.EXPORT_NAME)
    src = open(final).read()
...
def module_export_name_substitution():
    global final
    logger.debug('Private module export name substitution with ' +
                 shared.Settings.EXPORT_NAME)
    src = open(final).read()
    ...
    if shared.Settings.MINIMAL_RUNTIME:
        ...
        replacement = "typeof %(EXPORT_NAME)s !== 'undefined' ? %(EXPORT_NAME)s : {}" % {
            "EXPORT_NAME": shared.Settings.EXPORT_NAME}
    with open(final, 'w') as f:
        src = src.replace(
            shared.JS.module_export_name_substitution_pattern, replacement)
...
def generate_minimal_runtime_html(target, options, js_target, target_basename,
    ...
    if re.search('{{{\s*SCRIPT\s*}}}', shell):
        exit_with_error('--shell-file "' + options.shell_path +
    ...
    shell = tools.line_endings.convert_line_endings(
        shell, '\n', options.output_eol)
...
def generate_traditional_runtime_html(target, options, js_target, target_basename,
    ...
    if options.proxy_to_worker:
        proxy_worker_filename = (
            shared.Settings.PROXY_TO_WORKER_FILENAME or target_basename) + '.js'
...
''' % shared.JS.get_subresource_location(memfile)) + script.inline
        ...
        if not options.separate_asm or shared.Settings.WASM:
            if len(asm_mods):
                exit_with_error(
                    'no --separate-asm means no client code mods are possible')
        else:
            script.un_src()
            if len(asm_mods) == 0:
...
''' % (shared.JS.get_subresource_location(wasm_binary_target), script.inline)
    ...
    if script.inline:
        for filename in ('arrayUtils.js', 'base64Utils.js', 'URIUtils.js'):
            content = read_and_preprocess(
                shared.path_from_root('src', filename))
            script.inline = content + script.inline

        script.inline = 'var ASSERTIONS = %s;\n%s' % (
            shared.Settings.ASSERTIONS, script.inline)
    ...
    html_contents = tools.line_endings.convert_line_endings(
        html_contents, '\n', options.output_eol)
    with open(target, 'wb') as f:
...
def minify_html(filename, options):
    ...
    run_process(shared.NODE_JS + [shared.path_from_root('third_party',
                                                        'html-minifier', 'cli.js'), filename, '-o', filename] + opts)
    elapsed_time = time.time() - start_time
    size_after = os.path.getsize(filename)
    delta = size_after - size_before
    logger.debug('HTML minification took {:.2f}'.format(elapsed_time) + ' seconds, and shrunk size of ' + filename + ' from ' + str(
        size_before) + ' to ' + str(size_after) + ' bytes, delta=' + str(delta) + ' ({:+.2f}%)'.format(delta * 100.0 / size_before))
...
def generate_worker_js(target, js_target, target_basename):
    ...
    # compiler output goes in .worker.js file
        ...
        proxy_worker_filename = (
            shared.Settings.PROXY_TO_WORKER_FILENAME or worker_target_basename) + '.js'
...
def worker_js_script(proxy_worker_filename):
    web_gl_client_src = open(shared.path_from_root(
        'src', 'webGLClient.js')).read()
...
def parse_value(text):
    ...
    def parse_string_value(text):
        ...
        if first == "'" or first == '"':
            text = text.rstrip()
            assert text[-1] == text[0] and len(
                text) > 1, 'unclosed opened quoted string. expected final character to be "%s" and length to be greater than 1 in "%s"' % (text[0], text)
            return text[1:-1]
        return text
    ...
    def parse_string_list_members(text):
        ...
        index = 0
        while True:
            # Cannot safely rstrip for cases like: "HERE-> ,"
            current = values[index].lstrip()
            if not len(current):
                exit_with_error(
                    'string array should not contain an empty value')
            first = current[0]
            if not(first == "'" or first == '"'):
                ...
                while True:  # Continue until closing quote found
                    if index >= len(values):
                        exit_with_error("unclosed quoted string. expected final character to be '%s' in '%s'" % (
                            first, values[start]))
    ...
    def parse_string_list(text):
        ...
        if text[-1] != ']':
            exit_with_error(
                'unclosed opened string list. expected final character to be "]" in "%s"' % (text))
        inner = text[1:-1]
        if inner.strip() == "":
            return []
...
def validate_arg_level(level_string, max_level, err_msg, clamp=False):
    try:
        ...
        if clamp:
            if level > max_level:
                logger.warning("optimization level '-O" + level_string +
                               "' is not supported; using '-O" + str(max_level) + "' instead")
                level = max_level
